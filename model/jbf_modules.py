...
class PointWiseTCN(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, groups=1):
        ...
        self.conv = nn.Conv2d(
            in_channels, out_channels, 1, stride=(stride, 1), groups=groups
        )
        self.bn = nn.BatchNorm2d(out_channels)
...
class CTR_GC(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        q, k, v = (
            self.conv1(x).mean(-2),
            self.conv2(x).mean(-2),
            self.conv3(x).view(N, self.num_scale, self.Nh, -1, T, V),
        )
        weights = self.conv4(self.tanh(q.unsqueeze(-1) - k.unsqueeze(-2))).view(
            N, self.num_scale, self.Nh, -1, V, V
        )
        weights = weights * self.alpha.to(weights.dtype) + self.A.view(
            1, 1, self.Nh, 1, V, V
        ).to(weights.dtype)
        x = (
            torch.einsum("ngacvu, ngactu->ngctv", weights, v)
            .contiguous()
            .view(N, -1, T, V)
        )
        x = self.bn(x)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qk = y.chunk(2, dim=-1)
        q, k = map(
            lambda t: rearrange(t, "b t v (h d) -> (b t) h v d", h=self.n_heads), qk
        )
...
class DeTGC(nn.Module):
    ...
    def forward(self, x):
        ...
        tup = tdn + 1
        index1, index2 = (
            torch.clamp(tdn, 0, self.T - 1).long(),
            torch.clamp(tup, 0, self.T - 1).long(),
        )
...
class Info_Basic_Block(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(Info_Basic_Block, self).__init__()
