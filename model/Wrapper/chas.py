...
class CHASWrapper(nn.Module):
    def __init__(
        ...
        self.seg_num_list = [
            (num_frame // self.pooling_seg[0]),
            (num_point // self.pooling_seg[1]),
            (num_entity // self.pooling_seg[2]),
        ]
        self.seg_num = (
            self.seg_num_list[0] * self.seg_num_list[1] * self.seg_num_list[2]
        )
        self.shift = nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=c1, kernel_size=1),
            nn.AdaptiveAvgPool3d(
                (self.pooling_seg[0], self.pooling_seg[1], self.pooling_seg[2])
            ),
    ...
    def forward(self, x):
        ...
        sf = (
            (tx @ sf.softmax(dim=1))
            .unsqueeze(-1)
            .expand(-1, -1, self.seg, self.seg_num)
        )
        sf = rearrange(
            sf,
            "n c (T V M) (t v m) -> n c (T t) (V v) (M m)",
            T=self.pooling_seg[0],
            V=self.pooling_seg[1],
            M=self.pooling_seg[2],
            t=self.seg_num_list[0],
            v=self.seg_num_list[1],
            m=self.seg_num_list[2],
        ).contiguous()
        x = x - sf
        ...
        if self.training:
            ...
            sorted_indices, _ = torch.sort(
                torch.randperm(xt.size(-1))[:2].to(xt.device)
            )
