...
class DeTGCN(nn.Sequential):
    def __init__(self, block_args, A, k, eta):
        super(DeTGCN, self).__init__()
        for i, [
            in_channels,
            out_channels,
            stride,
...
class Model_CTRGCN(nn.Module):
    def __init__(
        ...
        super(Model_CTRGCN, self).__init__()
        ...
        self.data_bn = nn.BatchNorm1d(
            num_person * in_channels * num_point * self.num_modal
        )
        ...
        self.num_stream = 2
        self.streams1 = nn.ModuleList(
            [CTRGCN(self.blockargs1, A, k, eta) for _ in range(self.num_modal)]
        )
        self.streams2 = nn.ModuleList(
            [CTRGCN(self.blockargs2, A, k, eta) for _ in range(self.num_stream)]
        )
...
class Model_InfoGCN(nn.Module):
    def __init__(
        ...
        super(Model_InfoGCN, self).__init__()
        ...
        self.num_stream = 2
        self.streams1 = nn.ModuleList(
            [InfoGCN(self.blockargs1, A, k, eta) for _ in range(self.num_modal)]
        )
        self.streams2 = nn.ModuleList(
            [InfoGCN(self.blockargs2, A, k, eta) for _ in range(self.num_stream)]
        )
        self.fc = nn.ModuleList(
            [nn.Linear(base_channel * 4, num_class) for _ in range(self.num_stream)]
        )
...
class Model_DeTGCN(nn.Module):
    def __init__(
        ...
        super(Model_DeTGCN, self).__init__()
        ...
        self.num_stream = 2
        self.streams1 = nn.ModuleList(
            [DeTGCN(self.blockargs1, A, k, eta) for _ in range(self.num_modal)]
        )
        self.streams2 = nn.ModuleList(
            [DeTGCN(self.blockargs2, A, k, eta) for _ in range(self.num_stream)]
        )
        self.fc = nn.ModuleList(
            [nn.Linear(base_channel * 4, num_class) for _ in range(self.num_stream)]
        )
