...
class unit_gcn(nn.Module):
    ...
    def forward(self, x, get_topology=False, label=None, mix_top=False):
        ...
        if get_topology:
            ...
            return y, torch.stack(topology, 1)
        return y
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x, get_topology=True, label=None, mix_top=False):
        if self.use_pe:
            x = self.pos_enc(x)
        if get_topology:
            tmp, top = self.gcn1(x, get_topology, label=label, mix_top=mix_top)
            y = self.relu(self.tcn1(tmp) + self.residual(x))
            return y, top
        else:
            z = self.gcn1(x, label=label, mix_top=mix_top)
