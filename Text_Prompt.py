...
uav_label_map = []
with open("./text/uav_label_map.txt") as infile:
    lines = infile.readlines()
    for ind, line in enumerate(lines):
        uav_label_map.append(line.rstrip().lstrip())

uav_paste_text_map0 = []
with open("./text/uav_synonym_openai.txt") as infile:
    lines = infile.readlines()
    for ind, line in enumerate(lines):
        temp_list = line.rstrip().lstrip().split(',')
        uav_paste_text_map0.append(temp_list)

uav_paste_text_map2 = []
with open("./text/uav_pasta_openai.txt") as infile:
    lines = infile.readlines()
    for ind, line in enumerate(lines):
        temp_list = line.rstrip().lstrip().split(';')
        uav_paste_text_map2.append(temp_list)

uav_motion_details_map = []
with open("./text/uav_motion_details.txt") as infile:
    lines = infile.readlines()
    for ind, line in enumerate(lines):
        temp_list = line.rstrip().lstrip().split('.')
        uav_motion_details_map.append(temp_list)

uav_used_parts_map = []
with open('./text/uav_used_parts-openai.txt') as infile:
    lines = infile.readlines()
    for ind, line in enumerate(lines):
        temp_list = line.rstrip().lstrip().split(',')
        uav_used_parts_map.append(temp_list)


def get_global_text(dir=''):
    print('-'*26,'load global text from \'{}\''.format(dir),'-'*26)
    text_map = []
    ...
    total_list = []
    for pasta_list in text_map:
        temp_list = []
        for item in pasta_list:
            temp_list.append(clip.tokenize(item))
        # for i in range(1, len(pasta_list)):
        #    item = pasta_list[0] + 'can be described as' + pasta_list[i]
        #    temp_list.append(clip.tokenize(item))
        total_list.append(temp_list)
    return total_list

def get_global_text_token(dir=''):
    print('-'*26,'load global text from \'{}\''.format(dir),'-'*26)
    text_map = []
    with open(dir) as infile:
        lines = infile.readlines()
    ...
    for pasta_list in text_map:
        ...
        for i in range(1, len(pasta_list)):
           item = pasta_list[0] + ' can be described as' + pasta_list[i]
           # item = pasta_list[0] + ' ,' + pasta_list[i]
           temp_list.append(clip.tokenize(item))
        total_list.append(temp_list)
    return total_list
...
def uav_text_prompt_openai_random():
    print("Use text prompt openai synonym random")

    total_list = []
    for pasta_list in uav_paste_text_map0:
        temp_list = []
        for item in pasta_list:
            temp_list.append(clip.tokenize(item))
        total_list.append(temp_list)
...
def uav_motion_details():
    print("use the action's(in UAVHuman) motion details ")

    total_list = []
    for pasta_list in uav_motion_details_map:
        temp_list = []
        for item in pasta_list:
            temp_list.append(clip.tokenize(item))
        total_list.append(temp_list)
...
def uav_used_parts():
    print("used parts in one action")

    total_list = []
    for pasta_list in uav_used_parts_map:
        temp_list = []
        for item in pasta_list:
            temp_list.append(clip.tokenize(item))
        total_list.append(temp_list)
...
def uav_text_prompt_openai_pasta_pool_4part():
    print("Use text prompt openai pasta pool")
    text_dict = {}
    num_text_aug = 5
    for ii in range(num_text_aug):
        if ii == 0:
            text_dict[ii] = torch.cat([clip.tokenize((pasta_list[ii])) for pasta_list in uav_paste_text_map2])
        elif ii == 1:
            text_dict[ii] = torch.cat([clip.tokenize((','.join(pasta_list[0:2]))) for pasta_list in uav_paste_text_map2])
        elif ii == 2:
            text_dict[ii] = torch.cat([clip.tokenize((pasta_list[0] +','.join(pasta_list[2:4]))) for pasta_list in uav_paste_text_map2])
        elif ii == 3:
            text_dict[ii] = torch.cat([clip.tokenize((pasta_list[0] +','+ pasta_list[4])) for pasta_list in uav_paste_text_map2])
        else:
            text_dict[ii] = torch.cat([clip.tokenize((pasta_list[0]+','+','.join(pasta_list[5:]))) for pasta_list in uav_paste_text_map2])
